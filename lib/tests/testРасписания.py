...
class TestРасписание(unittest.TestCase):
    ...
    def testПереименованияЗадачи(self):
        задача1 = self.расписание.создатьЗадачу("Задача-1")
    ...
    def testВыполнениеВложенныхМинимумовПереводитСостояниеКонтейнераБезОграниченийВМожно(self):
        self.расписание.проверитьНаПредметКонцаЦикла = Mock()
        задача1 = self.расписание.создатьЗадачу("Задача-1")
        задача11 = задача1.создатьЗадачу("Задача-1-1", период(10), период(5))
        задача12 = задача1.создатьЗадачу("Задача-1-2", период(10), период(5))
        задача13 = задача1.создатьЗадачу("Задача-1-3", период(10), период(5))

        self.assertEqual(СостояниеЗадачи.нужноДелать, задача1.состояние())
        задача11.затратитьВремя(период(6))
        self.assertTrue(задача1.нужноДелать())

        задача12.затратитьВремя(период(6))
        self.assertTrue(задача1.нужноДелать())

        задача13.затратитьВремя(период(6))
        self.assertTrue(задача1.можноДелать())
