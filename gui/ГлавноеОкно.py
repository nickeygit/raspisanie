...
class ГлавноеОкно(MainWindow):
    ...
    def обновитьЭлемент(self, элемент):
        ...
        self.сброситьПредыдущееСостояниеЗадачи(элемент.задача)

    def сброситьПредыдущееСостояниеЗадачи(self, задача):
    ...
    def соединитьЭлементИЗадачу(self, элемент, задача):
        ...
        self.сброситьПредыдущееСостояниеЗадачи(задача)
    ...
    def затратитьВремяНаЗадачу(self, задача, период):
        ...
        self.рекурсивноОбновитьСостояниеГруппы(задача.элемент)

    def обновитьСостояниеЗадачи(self, задача):
        состояние = задача.состояние()
        предыдущееСостояние = задача.предыдущееСостояние
        self.обновитьСостояниеЗадачиБезСистемногоУведомления(задача)
        self.системноеУведомлениеПриСменеСостояния(задача, состояние, предыдущееСостояние)

    def обновитьСостояниеЗадачиБезСистемногоУведомления(self, задача):
        self.сброситьПредыдущееСостояниеЗадачи(задача)
        self.обновитьЗатраченноеВремяЗадачи(задача, задача.элемент)

    def системноеУведомлениеПриСменеСостояния(self, задача, состояние, предыдущееСостояние):
        if состояние != предыдущееСостояние:
            self.системныйЗначок.showMessage(текстСостояния[состояние], str(задача),
                                             QSystemTrayIcon.MessageIcon(QSystemTrayIcon.Warning), 0)
    ...
    def рекурсивноОбновитьСостояниеГруппы(self, элемент):
        while элемент is not None:
            self.обновитьСостояниеЗадачиБезСистемногоУведомления(элемент.задача)
